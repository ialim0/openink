...
const LayoutContent: React.FC<LayoutWrapperProps> = ({ children }) => {
  ...
  return (
    <body className={`h-screen flex flex-col ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        <aside className={`md:w-1/3 p-8 ${darkMode ? 'bg-gray-800' : 'bg-white'} border-r border-gray-200 shadow-lg flex flex-col justify-between ${menuVisible ? 'fixed inset-0 z-40' : 'hidden'} md:block md:relative md:h-full overflow-y-auto`}>
          <Menu />
        </aside>
